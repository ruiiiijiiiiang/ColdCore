...
def send_confirmation_email(team_email, confirmation_key):
    send_email(team_email, "Welcome to {}!".format(config.site_name),
...
""".format(config.site_name, config.domain_name, confirmation_key))
...
def send_password_reset_email(team_email, password_reset_token):
    send_email(team_email, "{} Password Reset".format(config.site_name),
...
""".format(config.domain_name, password_reset_token))
